...
<body class="font-sans antialiased bg-gray-100">
    ...
    @php
            ...
            function request() {
                ...
                return new class($currentRoute) {
                    ...
                    public function routeIs($pattern) {
                        ...
                        if ($pattern === 'dashboard' && $this->route === '/dashboard') return true;
                        if ($pattern === 'admin.propiedades.*' && str_starts_with($this->route, '/admin/propiedades')) return true;
                        if ($pattern === 'admin.contracts.*' && str_starts_with($this->route, '/admin/contracts')) return true;
                        if ($pattern === 'admin.users' && $this->route === '/admin/users') return true;
                        if ($pattern === 'admin.messages' && str_starts_with($this->route, '/admin/messages')) return true;
                        if ($pattern === 'admin.messages.show' && str_starts_with($this->route, '/admin/messages/')) return true;
                        return false;
                    }
        ...
        if (!class_exists('MockUser')) { 
            class MockUser { public $name = 'Juan Administrador'; public $email = 'admin@rv.com'; }
        }
        if (!class_exists('MockAuth')) { 
            class MockAuth { public static function user() { return new MockUser(); } }
        }
        if (!class_exists('Auth')) { class Auth extends MockAuth {} }

        // Definición de clases para el sidebar
        $navClasses = 'flex items-center py-2.5 px-4 rounded-lg transition duration-200';
        // CLASES ACTIVAS CAMBIADAS A ROJO (red-600)
            ...
            <nav class="space-y-2">
                ...
                <a href="{{ route('dashboard') }}" 
                   class="{{ $navClasses }} @if(request()->routeIs('dashboard')) {{ $activeClasses }} @else {{ $inactiveClasses }} @endif">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-10v10a1 1 0 001 1h3m-9 0V9a1 1 0 00-1-1H7a1 1 0 00-1 1v10m9-10l-7 7"></path></svg>
                    Dashboard
                </a>
                
                {{-- Propiedades --}}
                <a href="{{ route('admin.properties.index') }}" 
                   class="{{ $navClasses }} @if($isPropiedadesActive) {{ $activeClasses }} @else {{ $inactiveClasses }} @endif">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-1 0H7m0 0h-2M5 5v16m0-4h14"></path></svg>
                    Propiedades
                </a>
                ...
                <a href="{{ route('admin.users') }}" 
                   class="{{ $navClasses }} @if(request()->routeIs('admin.users')) {{ $activeClasses }} @else {{ $inactiveClasses }} @endif">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 15L11 19m0 0l-4-4m4 4v7"></path></svg>
                    Usuarios
                </a>
                
                {{-- Mensajes --}}
                <a href="{{ route('admin.messages') }}" 
                   class="{{ $navClasses }} @if(request()->routeIs('admin.messages') || request()->routeIs('admin.messages.show')) {{ $activeClasses }} @else {{ $inactiveClasses }} @endif">
                ...
                {{-- Cerrar Sesión --}}
                <form method="POST" action="{{ route('logout') }}" class="pt-4 border-t border-gray-200 mt-4">
                    @csrf
                    <button type="submit" class="flex items-center py-2.5 px-4 rounded-lg transition duration-200 bg-red-600 hover:bg-red-700 w-full text-left font-medium text-white">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white shadow p-4 flex justify-between items-center z-10">
                ...
                <div class="flex items-center space-x-4">
                    <p class="text-sm text-gray-700">Bienvenido, <span class="font-semibold">{{ Auth::user()->name ?? 'Usuario' }}</span></p>
                </div>
            </header>
