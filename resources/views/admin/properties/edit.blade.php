...
@section('content')
    @php
        ...
        $property = [
            ...
            'description' => 'Luminoso apartamento en el corazón de la ciudad, recién remodelado y con excelente transporte público.',
            'price' => 250000000,
            'bedrooms' => 3,
            'bathrooms' => 2,
    ...
    <h2 class="text-3xl font-bold mb-6 text-gray-800">Editar Propiedad: #{{ $property['id'] }} - {{ $property['title'] }}</h2>
        ...
        <form action="{{ route('admin.properties.update', $property['id']) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500">
                        <option value="Disponible" {{ old('status', $property['status']) == 'Disponible' ? 'selected' : '' }}>Disponible</option>
                        <option value="Alquilado" {{ old('status', $property['status']) == 'Alquilado' ? 'selected' : '' }}>Alquilado</option>
            ...
            <div class="mt-8 space-y-6">
                ...
                <div>
                    <label for="main_image" class="block text-sm font-medium text-gray-700 mb-1">Cambiar Imagen Principal</label>
